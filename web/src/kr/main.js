...
async function loadAllSpecs(container) {
    try {
        ...
        if (response.ok && result.success) {
            ...
            container.innerHTML = `<p class="text-danger text-center w-100 py-4">서버 스펙 목록을 불러오지 못했습니다.</p>`;
        }
    } catch (e) {
        console.error(e);
        container.innerHTML = `<p class="text-danger text-center w-100 py-4">서버와의 연결에 실패했습니다.</p>`;
    }
}
